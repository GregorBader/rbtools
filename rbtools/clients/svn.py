...
class SVNClient(SCMClient):
    ...
    INDEX_SEP = b'=' * 67
    INDEX_FILE_RE = re.compile(b'^Index: (.+?)(?:\t\((added|deleted)\))?\r?\n$')
    ...
    def normalize_patterns(self, patterns, base_dir, cwd):
        """Normalize the patterns so that they are all SVN paths.
    ...
    def diff(self, revisions, include_files=[], exclude_patterns=[],
        ...
        # paths.
        exclude_patterns = self.normalize_patterns(exclude_patterns,
                                                   '/',
    ...
    def parse_filename_header(self, diff_line):
        ...
        if b'\r' in diff_line:
            # Strip off carriage return, and return it with the rest of the line.
            parts = diff_line.split(b'\r', 1)
            return (parts[0].decode(_fs_encoding), b'\r' + parts[1])
